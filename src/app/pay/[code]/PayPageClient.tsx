...
export default function PayPageClient({
  ...
  const [member, setMember] = useState<BillMember | null>(initialMember);
  ...
  // If no member found and no guest name yet → show GuestNameInput
  if (!member && !guestName) {
    return (
      <div className="min-h-dvh bg-bg-primary flex flex-col">
        ...
        <div className="px-4 pt-12 pb-4 flex flex-col gap-1">
          ...
          <h1 className="font-syne font-bold text-xl text-text-primary">{bill.title}</h1>
        ...
        <div className="px-4 flex-1 flex flex-col justify-center">
          <GuestNameInput
            onComplete={(name) => setGuestName(name)}
          />
        </div>
      </div>
    );
        ...
        <AnimatePresence mode="wait">
          ...
          {equalStep === "entry" && (
            ...
            >
              ...
              <div>
                <h1 className="font-syne font-bold text-2xl text-text-primary">{bill.title}</h1>
                {organizerProfile?.name && (
                  <p className="text-text-muted text-sm font-dm">dari {organizerProfile.name}</p>
                )}
                {bill.description && (
              ...
              {/* Greeting + amount */}
              <div className="surface-card rounded-card p-5 flex flex-col items-center gap-2 text-center">
                ...
                <p className="font-syne font-extrabold text-5xl text-accent mt-1">
                  {formatRM(amountOwed)}
                </p>
              </div>
              ...
              <button
                ...
                className="bg-accent text-bg-primary font-dm font-semibold py-4 rounded-btn text-base w-full"
              >
                Buat Bayaran
              </button>
              ...
              >
                ...
                <div className="flex gap-2 mb-4">
                  {(["bank", "qr"] as PaymentTab[]).map((tab) => (
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-btn text-sm font-dm font-medium transition-colors ${
                        paymentTab === tab
                ...
                {paymentTab === "bank" ? (
                  <div className="flex flex-col gap-3">
                    <div className="bg-bg-primary rounded-input px-4 py-4 flex flex-col gap-1.5">
                ...
                <button
                  ...
                  className="w-full mt-4 bg-accent text-bg-primary font-dm font-semibold py-4 rounded-btn text-sm"
                >
                  Dah Transfer? →
                </button>
              </BottomSheet>
          ...
          {equalStep === "swipe" && (
            ...
            >
              <button
                ...
                className="flex items-center gap-2 text-text-secondary self-start"
              >
                <ArrowLeft size={18} />
              ...
              <div className="surface-card rounded-card p-5 flex flex-col items-center gap-2 text-center">
                <span
                  className="text-xs font-dm px-3 py-1 rounded-pill bg-warning/20 text-warning"
                >
                  Menunggu pengesahan
                </span>
                <p className="font-syne font-extrabold text-5xl text-accent mt-2">
                  {formatRM(amountOwed)}
                </p>
              ...
              <div className="surface-card rounded-card p-4 flex flex-col gap-3">
                <p className="text-text-secondary text-sm font-dm">
                  Screenshot Resit Transfer{" "}
                  <span className="text-text-muted">(pilihan)</span>
                </p>
                {screenshot ? (
                  <div className="flex items-center gap-3 bg-bg-primary rounded-input px-3 py-2">
                    <Check size={14} className="text-success" />
                    <span className="text-text-primary text-xs font-dm flex-1 truncate">
                      {screenshot.name}
                    </span>
                    <button onClick={() => setScreenshot(null)} className="text-danger">
                      <X size={14} />
                    </button>
                  </div>
                ) : (
                  <label className="flex items-center justify-center gap-2 border border-dashed border-white/20 rounded-input py-3 cursor-pointer text-text-secondary text-sm font-dm">
                    <Upload size={16} />
          ...
          <div className="flex items-center justify-center gap-2 text-xs font-dm">
            {(["tuntut", "semak", "bayar"] as const).map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                ...
                >
                  {i + 1}{" "}
                  {s === "tuntut" ? "Tuntut" : s === "semak" ? "Semak" : "Bayar"}
                </span>
              </div>
            ))}
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {scanStep === "tuntut" && (
          ...
          >
            ...
            {/* Items placeholder — ItemClaimList component */}
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-text-muted text-xs font-dm">
                      {item.qty}× {formatRM(item.edited_price)}
                    </p>
                  </div>
                  <span className="text-accent font-dm text-sm font-semibold">
                    {formatRM(item.edited_price * item.qty)}
                  </span>
                </div>
              ))}
              {(bill.bill_items ?? []).length === 0 && (
                <p className="text-text-muted text-sm font-dm text-center py-4">
                  Tiada item dalam resit
                </p>
              )}
            </div>
            ...
            <button
              ...
              className="bg-accent text-bg-primary font-dm font-semibold py-4 rounded-btn text-sm w-full"
            >
              Seterusnya →
            </button>
          </motion.div>
        ...
        {scanStep === "semak" && (
          ...
          >
            ...
            <h1 className="font-syne font-bold text-xl text-text-primary">Semak Resit</h1>
            <p className="text-text-secondary text-sm font-dm">
              Bandingkan caj dengan resit asal. Flag jika terdapat perbezaan.
            </p>

            {/* ComparisonTable placeholder */}
            <div className="surface-card rounded-card p-4">
              <div className="flex gap-2 mb-3">
                {["Ringkasan", "Resit Asal"].map((tab) => (
                  <button
                    key={tab}
                    className="px-3 py-1.5 rounded-pill text-xs font-dm bg-bg-primary text-text-secondary border border-white/10"
                  >
                    {tab}
                  </button>
                ))}
        ...
        {scanStep === "bayar" && (
          ...
          >
            <button
              onClick={() => setScanStep("semak")}
              className="flex items-center gap-2 text-text-secondary self-start mt-4"
            >
              <ArrowLeft size={18} />
              <span className="text-sm font-dm">Kembali</span>
            </button>

            {/* Amount breakdown (collapsible hint) */}
            <div className="surface-card rounded-card p-5 flex flex-col items-center gap-2 text-center">
              <span className="text-xs font-dm px-3 py-1 rounded-pill bg-warning/20 text-warning">
                Menunggu pengesahan
              </span>
              <p className="font-syne font-extrabold text-5xl text-accent mt-2">
                {formatRM(amountOwed)}
              ...
              <p className="text-text-secondary font-dm text-sm">{bill.title}</p>
              <p className="text-text-muted text-xs font-dm">
                Berdasarkan item yang anda tuntut
              </p>
            </div>
...
function SuccessScreen({
  ...
  return (
    ...
    >
      {/* Success card */}
      <div className="surface-card rounded-card p-6 w-full flex flex-col items-center gap-3 text-center">
        <div className="w-20 h-20 rounded-full bg-success/20 flex items-center justify-center">
          <Check size={36} className="text-success" strokeWidth={2.5} />
        </div>
        <h2 className="font-syne font-bold text-2xl text-text-primary">Bayaran Disahkan!</h2>
        <p className="text-text-secondary font-dm text-sm">
          Terima kasih, <span className="text-text-primary font-semibold">{name}</span>!
        </p>

        <div className="w-full bg-bg-primary rounded-input px-4 py-3 flex flex-col gap-2 mt-2">
          <div className="flex justify-between">
            <span className="text-text-muted text-xs font-dm">Bil</span>
            <span className="text-text-primary text-xs font-dm font-medium truncate max-w-[60%] text-right">
              {billTitle}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-muted text-xs font-dm">Jumlah</span>
            <span className="text-success font-dm font-bold text-sm">
              {formatRM(amountOwed)}
            </span>
          </div>
          <div className="flex justify-between">
        ...
        <p className="text-text-muted text-xs font-dm mt-1">
          Screenshot dan simpan sebagai bukti
        </p>
      </div>
      ...
      {!dismissPromo && (
        <motion.div
          ...
          className="surface-card rounded-card p-5 w-full flex flex-col gap-3 relative"
        >
          <button
          ...
          <h3 className="font-syne font-bold text-text-primary text-base pr-6">
            Nak jadi organizer? 🚀
          </h3>
          <p className="text-text-secondary font-dm text-sm">
          ...
          <Link
            href="/auth/register"
            className="flex items-center justify-center bg-accent text-bg-primary font-dm font-semibold py-3 rounded-btn text-sm"
          >
            Daftar Percuma →
          </Link>
        </motion.div>
